        ...
        var list = ["A", "F", "G", "H", "J", "L", "N", "Q", "S", "X", "Y", "Z", " "]; for (var i = 0; i < list.length; i++) {
            ...
            for (var provinceName in provinceList) {
                if (provinceList[provinceName] === list[i]) {
                    ...
                    for (cityName in cityList[provinceName]) {
...
})(); function nearcity(data) { $("#header-recommend").html("周边城市推荐:&nbsp;"); if (catepath === "post") { for (var i = 0; i < data.length; i++) { $("#header-recommend").append($('<a href="//post.zgxywhw2014.com/' + data[i].cityid + '/" class="recommend-city">' + data[i].name + "</a>")) } } else { for ( i = 0; i < data.length; i++) { $("#header-recommend").append($('<a href="//' + data[i].city + ".zgxywhw2014.com/" + catepath + '" class="recommend-city">' + data[i].name + "</a>")) } } } var _trackURL = "{'cate':'" + window.fullpath + "','area':'','pagetype':'changecity','page':''}";